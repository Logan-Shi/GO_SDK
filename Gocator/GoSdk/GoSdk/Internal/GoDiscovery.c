...
GoFx(kStatus) GoDiscovery_GetReceivedResponseId(GoDiscovery discovery, k64s* responseId)
{
    ...
    kTry
    {
        ...
        kTest(kSerializer_EndRead(obj->reader));
        needEndRead = kFALSE;

        // Take whatever response id value was read from the reply and return it
        // to the caller to determine if it is valid in the caller's context.
        // Actual values only require 32-bits so the casting from 64-bits
        // to 32-bits should be safe.
    ...
    kCatchEx(&exception)
    {
        ...
        // Release serializer allocated memory.
        if (needEndRead)
        {
            kSerializer_EndRead(obj->reader);
        }
