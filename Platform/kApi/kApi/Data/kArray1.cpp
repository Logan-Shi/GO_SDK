...
kBeginClassEx(k, kArray1) 
    ...
    kAddPrivateVMethod(kArray1, kObject, VInitCloneEx)
    ...
    kAddPrivateIVMethod(kArray1, kArrayProvider, VDefaultConstruct, DefaultConstruct)
...
kFx(kStatus) kArray1_ConstructEx(kArray1* array, kType itemType, kSize length, kAlloc allocator, kAlloc valueAllocator)
{
    ...
    if (!kSuccess(status = xkArray1_Init(*array, kTypeOf(kArray1), itemType, length, alloc, valueAlloc)))
    {
...
kFx(kStatus) xkArray1_DefaultConstruct(kArray1* array, kAlloc objectAlloc, kAlloc valueAlloc)
{
    return kArray1_ConstructEx(array, kNULL, 0, objectAlloc, valueAlloc);
}

kFx(kStatus) xkArray1_Init(kArray1 array, kType classType, kType itemType, kSize length, kAlloc alloc, kAlloc valueAlloc)
{
    kObjR(kArray1, array);
...
kFx(kStatus) xkArray1_VInitCloneEx(kArray1 array, kArray1 source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kArray1, array);
    ...
    kTry
    {
        kTest(xkArray1_Imitate(array, source));     

        kTest(kCloneItems(obj->itemType, kArray1_DataAlloc(array), obj->items, kArray1_DataAlloc(source), sourceObj->items, obj->length, context, objectAlloc, valueAlloc)); 
    }
    kCatch(&status)
    {
...
kFx(kStatus) xkArray1_Realloc(kArray1 array, kSize length)
{
    ...
    if (newSize > obj->allocSize)
    {
        ...
        kCheck(kAlloc_Get(kArray1_DataAlloc(array), newSize, &newItems));
