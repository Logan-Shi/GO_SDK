...
kBeginClassEx(k, kArray2) 
    ...
    kAddPrivateVMethod(kArray2, kObject, VInitCloneEx)
    ...
    kAddPrivateIVMethod(kArray2, kArrayProvider, VDefaultConstruct, DefaultConstruct)
...
kFx(kStatus) kArray2_ConstructEx(kArray2* array, kType itemType, kSize length0, kSize length1, kAlloc allocator, kAlloc valueAllocator)
{
    ...
    if (!kSuccess(status = xkArray2_Init(*array, kTypeOf(kArray2), itemType, length0, length1, alloc, valueAlloc)))
    {
...
kFx(kStatus) xkArray2_DefaultConstruct(kArray2* array, kAlloc objectAlloc, kAlloc valueAlloc)
{
    return kArray2_ConstructEx(array, kNULL, 0, 0, objectAlloc, valueAlloc);
}

kFx(kStatus) xkArray2_Init(kArray2 array, kType classType, kType itemType, kSize length0, kSize length1, kAlloc alloc, kAlloc valueAlloc)
{
    kObjR(kArray2, array);
...
kFx(kStatus) xkArray2_VInitCloneEx(kArray2 array, kArray2 source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kArray2, array);
    ...
    kTry
    {
        kTest(xkArray2_Imitate(array, source));     

        kTest(kCloneItems(obj->itemType, kArray2_DataAlloc(array), obj->items, kArray2_DataAlloc(source), sourceObj->items, count, context, objectAlloc, valueAlloc));  
    }
    kCatch(&status)
    {
...
kFx(kStatus) xkArray2_Realloc(kArray2 array, kSize length0, kSize length1)
{
    ...
    if (newSize > obj->allocSize)
    {
        ...
        kCheck(kAlloc_Get(kArray2_DataAlloc(array), newSize, &newItems));
