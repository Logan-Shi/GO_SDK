 ...
 *
 * @internal
 * Copyright (C) 2006-2020 by LMI Technologies Inc.
...
kBeginClassEx(k, kArray3) 
    ...
    kAddPrivateVMethod(kArray3, kObject, VInitCloneEx)
    ...
    kAddPrivateIVMethod(kArray3, kArrayProvider, VDefaultConstruct, DefaultConstruct)
...
kFx(kStatus) kArray3_ConstructEx(kArray3* array, kType itemType, kSize length0, kSize length1, kSize length2, kAlloc allocator, kAlloc valueAllocator)
{
    ...
    if (!kSuccess(status = xkArray3_Init(*array, kTypeOf(kArray3), itemType, length0, length1, length2, alloc, valueAlloc)))
    {
...
kFx(kStatus) xkArray3_DefaultConstruct(kArray3* array, kAlloc objectAlloc, kAlloc valueAlloc)
{
    return kArray3_ConstructEx(array, kNULL, 0, 0, 0, objectAlloc, valueAlloc);
}

kFx(kStatus) xkArray3_Init(kArray3 array, kType classType, kType itemType, kSize length0, kSize length1, kSize length2, kAlloc alloc, kAlloc valueAlloc)
{
    kObjR(kArray3, array);
...
kFx(kStatus) xkArray3_VInitCloneEx(kArray3 array, kArray3 source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kArray3, array);
    kObjN(kArray3, sourceObj, source);
    kSize count = kArray3_Count(source); 
    kStatus status = kOK; 

    kCheck(kObject_Init(array, kObject_Type(source), objectAlloc));     

    obj->valueAlloc = valueAlloc; 
    obj->itemType = kTypeOf(kVoid); 
    obj->itemSize = 0; 
    obj->allocSize = 0; 
    obj->items = kNULL; 
    obj->length[0] = 0; 
    ...
    kTry
    {
        kTest(xkArray3_Imitate(array, source));     

        kTest(kCloneItems(obj->itemType, kArray3_DataAlloc(array), obj->items, kArray3_DataAlloc(source), sourceObj->items, count, context, objectAlloc, valueAlloc));  
    }
    kCatch(&status)
    {
...
kFx(kStatus) xkArray3_Realloc(kArray3 array, kSize length0, kSize length1, kSize length2)
{
    ...
    if (newSize > obj->allocSize)
    {
        ...
        kCheck(kAlloc_Get(kArray3_DataAlloc(array), newSize, &newItems));
