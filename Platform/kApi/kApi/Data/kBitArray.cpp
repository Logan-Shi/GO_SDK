 ...
 *
 * @internal
 * Copyright (C) 2019-2020 by LMI Technologies Inc.
...
kBeginClassEx(k, kBitArray) 
    ...
    kAddPrivateVersion(kBitArray, "kdat6", "7.0.0.0", "kBitArray-0", WriteDat6V0, ReadDat6V0)

    //virtual methods
    kAddPrivateVMethod(kBitArray, kObject, VRelease)
    kAddPrivateVMethod(kBitArray, kObject, VInitCloneEx)
...
kFx(kStatus) xkBitArray_VInitCloneEx(kBitArray array, kBitArray source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kBitArray, array);
    kStatus status = kOK; 
    ...
    kTry
    {
        kTest(kBitArray_Assign(array, source)); 
    }
    kCatch(&status)
    {
        xkBitArray_VRelease(array); 
...
kFx(kStatus) xkBitArray_ReadDat6V0(kBitArray array, kSerializer serializer, kAlloc allocator)
{
    kObjR(kBitArray, array);
    kStatus status; 
    kSize bitLength = 0; 
    ...
    kCheck(xkBitArray_Init(array, kTypeOf(kBitArray), bitLength, allocator)); 

    kTry
    {
        kTest(kSerializer_Read64uArray(serializer, obj->buffer,  xkBitArray_WordCount(array))); 
    }
    kCatch(&status)
    {
        xkBitArray_VRelease(array); 
