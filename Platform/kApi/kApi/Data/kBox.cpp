...
kBeginClassEx(k, kBox) 
    ...
    kAddPrivateVMethod(kBox, kObject, VInitCloneEx)
...
kFx(kStatus) xkBox_VInitCloneEx(kBox box, kBox source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kBox, box);
    kObjN(kBox, sourceObj, source);
    kStatus status = kOK; 

    kCheck(kObject_Init(box, kObject_Type(source), objectAlloc));     
