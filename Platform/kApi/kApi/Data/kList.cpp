 ...
 *
 * @internal
 * Copyright (C) 2013-2020 by LMI Technologies Inc.
...
kBeginClassEx(k, kList) 
    ...
    kAddPrivateVMethod(kList, kObject, VInitCloneEx)
...
kFx(kStatus) xkList_VInitCloneEx(kList list, kList source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{ 
    kObjR(kList, list);
    ...
    kCheck(xkList_Init(list, kObject_Type(source), sourceObj->contentField.type, sourceObj->count, objectAlloc)); 
