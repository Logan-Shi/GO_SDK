 ...
 *
 * @internal
 * Copyright (C) 2012-2020 by LMI Technologies Inc.
...
kBeginClassEx(k, kMap) 
    ...
    kAddPrivateVMethod(kMap, kObject, VInitCloneEx)
...
kFx(kStatus) xkMap_VInitCloneEx(kMap map, kMap source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{ 
    kObjR(kMap, map); 
    ...
    kCheck(xkMap_Init(map, kObject_Type(source), sourceObj->keyField.type, sourceObj->valueField.type, sourceObj->count, objectAlloc)); 
