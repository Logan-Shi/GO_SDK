...
kBeginClassEx(k, kQueue) 
    ...
    kAddPrivateVMethod(kQueue, kObject, VInitCloneEx)
...
kFx(kStatus) xkQueue_VInitCloneEx(kQueue queue, kQueue source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kQueue, queue);  
    ...
    kCheck(kObject_Init(queue, kObject_Type(source), objectAlloc));     
    ...
    kTry
    {
        ...
        kTest(kCloneItems(obj->itemType, kObject_Alloc(queue), kQueue_DataAt(queue, 0), kObject_Alloc(source), kQueue_DataAt(source, 0), headCount, context, objectAlloc, valueAlloc)); 
        kTest(kCloneItems(obj->itemType, kObject_Alloc(queue), kQueue_DataAt(queue, headCount), kObject_Alloc(source), kQueue_DataAt(source, headCount), tailCount, context, objectAlloc, valueAlloc)); 
    }
    kCatch(&status)
    {
