...
kBeginClassEx(k, kString) 
    ...
    kAddPrivateVMethod(kString, kObject, VInitCloneEx)
...
kFx(kStatus) xkString_VInitCloneEx(kString str, kString source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kString, str); 
    kStatus status = kOK; 

    kCheck(kObject_Init(str, kObject_Type(source), objectAlloc)); 
