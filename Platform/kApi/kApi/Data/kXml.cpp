 ...
 *
 * @internal
 * Copyright (C) 2004-2020 by LMI Technologies Inc.
...
kBeginClassEx(k, kXml)
    ...
    kAddPrivateVMethod(kXml, kObject, VInitCloneEx)
kEndClassEx() 
...
kFx(kStatus) xkXml_VInitCloneEx(kXml xml, kXml source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kStatus status; 
    
    kCheck(xkXml_Init(xml, objectAlloc));
