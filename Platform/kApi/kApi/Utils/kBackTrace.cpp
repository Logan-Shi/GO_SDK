 ...
 *
 * @internal
 * Copyright (C) 2014-2020 by LMI Technologies Inc.
...
kBeginClassEx(k, kBackTrace)
    kAddPrivateVMethod(kBackTrace, kObject, VInitCloneEx)
...
kFx(kStatus) xkBackTrace_VInitCloneEx(kBackTrace trace, kBackTrace source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kBackTrace, trace); 
    kObjN(kBackTrace, sourceObj, source);

    kCheck(kObject_Init(trace, kTypeOf(kBackTrace), objectAlloc)); 
