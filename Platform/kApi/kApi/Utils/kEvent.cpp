...
kBeginClassEx(k, kEvent)
    ...
    kAddPrivateVMethod(kEvent, kObject, VInitCloneEx)
kEndClassEx()
...
kFx(kStatus) xkEvent_VInitCloneEx(kEvent evnt, kEvent source, kAlloc objectAlloc, kAlloc valueAlloc, kObject context)
{
    kObjR(kEvent, evnt); 
    kStatus status = kOK; 

    kCheck(xkEvent_Init(evnt, kObject_Type(source), objectAlloc)); 
